test_that("ggpackets prints object name in console output", {
  expect_output(print(ggpacket()), "<ggpacket>")
})

test_that("ggpackets prints section for bound data in console output", {
  expect_output(print(ggpacket()), "Data")
})

test_that(
  paste(
    "ggpackets without bound data print awaiting message in console",
    "output"
  ),
  expect_output(print(ggpacket()), "awaiting data")
)
...
test_that(
  "ggpacket prints section for bound aesthetics mapping in console output",
  expect_output(print(ggpacket()), "Aesthetic Mapping")
)

test_that(
  "ggpacket with bound, empty aesthetic mapping prints empty aesthetics",
  expect_output(print(ggpacket()), trimws(capture.output(aes())[-1]))
)
...
test_that(
  "ggpacket without ggcall layers displays empty layers in console output",
  expect_output(print(ggpacket()), "Layers:\\n\\s*empty")
)
...
test_that(
  "ggpacket including required aesthetics considers internal remappings",
  {
    ggpk <- ggpacket(aes(y = test)) + geom_line(aes(x = ..y..))
    expect_silent(out <- capture.output(ggpk))
    expect_true(!any(grepl("missing", out)))
  }
)

test_that(
  "show(<ggpacket>) behaviors identical to print for command line output",
  {
    expect_identical(
