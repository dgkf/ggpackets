...
library(Hmisc) # for ggplot stat summary functions
library(ggplot2)
```
...
sacrafice a lot of the customizability of building our plot from scratch for 
...
`ggpacket` aims to reduce this inevitable issue by making it as simple as
...
ggplot(mtcars) + 
  aes(x = carb, y = mpg) + 
  my_ggpk(fun.data = mean_se, bar.fill = carb)
```

> **Tip:**
> 
> When multiple arguments are passed to `ggpack()` with the same name, only the
last will be used.
>
>  * `ggpack(geom_bar, fill = 'red') # will be red`
>  * `ggpack(geom_bar, fill = 'red', dots = list(fill = 'blue')) # blue`
>  * `ggpack(geom_bar, fill = 'red', dots = list(fill = 'blue'), fill = 'green') # green`
...
```{r ex1.ggpackets.ex1, warning = FALSE}
ggplot(mtcars) + 
...
```{r ex1.ggpackets.ex2, warning = FALSE}
ggplot(mtcars) + 
...
```{r ex1.ggpackets.ex3, warning = FALSE}
my_ggpk2 <- ggpacket() + 
...
```{r ex1.ggpackets.ex4, warning = FALSE}
ggplot(mtcars) + 
